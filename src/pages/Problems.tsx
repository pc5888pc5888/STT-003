...
export default function Problems() {
  ...
  return (
    <div className="stt-problems-page bg-[#fbfaf7] text-[#26231f]">
      <FullBleedHero theme="problems" />
      <section className="px-6 py-12 lg:px-10 lg:py-18" id="problem-index" aria-label="八個問題入口"><div className="mx-auto max-w-[1180px] divide-y divide-[#ded4c4] border-y border-[#ded4c4]">{problems.map((problem,index)=><article key={problem.id} id={problem.id} className="grid gap-6 py-9 lg:grid-cols-[90px_1.25fr_1fr] lg:gap-10 lg:py-12"><div className="font-serif text-2xl text-[#b18a54]">{String(index+1).padStart(2,"0")}</div><div><h2 className="font-serif text-2xl leading-10 lg:text-3xl">{problem.title}</h2><p className="stt-problem-misjudgment mt-4 text-sm leading-7 text-[#70695f]"><span className="stt-problem-label">常見誤判｜</span>{problem.misjudgment}</p></div><div className="flex flex-col justify-between gap-6 border-l-0 border-[#d8c8ad] lg:border-l lg:pl-8"><p className="stt-problem-result text-sm leading-7 text-[#5f594f]"><span className="stt-problem-label text-[#9c7440]">可形成結果｜</span>{problem.result}</p><div className="flex flex-wrap gap-5"><button type="button" onClick={()=>navigate(`/problems/${problem.id}`)} className="w-fit border-b border-[#a37a43] bg-transparent pb-1 text-sm text-[#8c6535]">先看完整判讀 →</button><button type="button" onClick={()=>navigate(`/start?route=${problem.id}`)} className="w-fit border-0 bg-transparent pb-1 text-sm text-[#81766a]">直接開始</button></div></div></article>)}</div></section>
