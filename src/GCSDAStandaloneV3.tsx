...
function PageHead({eyebrow,title,lead}:{eyebrow:string;title:string;lead:string}){const lines=splitBalancedTitle(title);const loc=useLocation();const visual=pageVisuals[loc.pathname];return <section className="g4-pagehead">{visual?<img className="g4-pagehead-media" src={visual} alt="" aria-hidden="true"/>:<div className="g4-pagehead-linework" aria-hidden="true"><span/><span/><span/></div>}<div className="g4-pagehead-veil" aria-hidden="true"/><div className="g4-wrap g4-pagehead-inner"><div className="g4-pagehead-copy"><div className="g4-kicker">{eyebrow}</div><h1 aria-label={title}>{lines.map((line,i)=><span className="g4-title-line" key={i}>{line}</span>)}</h1><p>{lead}</p></div></div></section>}
