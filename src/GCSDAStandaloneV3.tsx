...
function Shell({children}:{children:ReactNode}){
  ...
  return <div className="g4"><Styles/><header className="g4-head"><div className="g4-wrap g4-headin"><Link className="g4-brand" to="/" aria-label="GCSDA 首頁"><b>GCSDA</b><small>中華企業策略永續發展學會</small></Link><nav className="g4-nav" aria-label="主要導覽">{navItems.map(([l,p])=><NavLink key={p} className={({isActive})=>isActive?"active":""} to={p}>{l}</NavLink>)}<a className="g4-stt" href={STT_URL} target="_blank" rel="noreferrer">STT Governance ↗</a></nav><button className="g4-menu" onClick={()=>setOpen(true)} aria-label="開啟選單" aria-expanded={open}><Menu/></button></div></header>{open&&<div className="g4-drawer" onClick={()=>setOpen(false)}><div className="g4-drawerpanel" onClick={e=>e.stopPropagation()}><button className="g4-close" onClick={()=>setOpen(false)} aria-label="關閉選單"><X/></button>{navItems.map(([l,p])=><Link key={p} to={p} onClick={()=>setOpen(false)}>{l}</Link>)}<a href={STT_URL} target="_blank" rel="noreferrer" onClick={()=>setOpen(false)}>STT Governance ↗</a></div></div>}<main>{children}</main><footer className="g4-footer"><div className="g4-wrap g4-footergrid"><div><b>中華企業策略永續發展學會｜GCSDA</b><p>依法成立之全國性專業社團。以公司治理法遵、企業策略、風險控管、跨界交流與永續發展為核心，逐步累積正式組織與治理知識。</p></div><div><a href={LINE_URL} target="_blank" rel="noreferrer">會員與聯絡</a><a href={STT_URL} target="_blank" rel="noreferrer">STT Governance ↗</a><Link to="/privacy">隱私</Link></div></div></footer></div>
}

function PageHead({eyebrow,title,lead}:{eyebrow:string;title:string;lead:string}){const lines=splitBalancedTitle(title);const loc=useLocation();const visual=pageVisuals[loc.pathname];const density=title.length>=34?" is-long":title.length>=20?" is-medium":" is-short";const visualState=visual?" has-image":" is-text-only";return <section className={"g4-pagehead"+density+visualState}>{visual?<img className="g4-pagehead-media" src={visual} alt="" aria-hidden="true"/>:null}<div className="g4-pagehead-veil" aria-hidden="true"/><div className="g4-wrap g4-pagehead-inner"><div className="g4-pagehead-copy"><div className="g4-kicker">{eyebrow}</div><h1 aria-label={title}>{lines.map((line,i)=><span className="g4-title-line" key={i}>{line}</span>)}</h1><p>{lead}</p></div></div></section>}
...
function Membership(){return <div className="g4-page"><PageHead eyebrow="MEMBERSHIP" title="加入學會，是進入一個以策略、治理、法遵與永續為共同語言的專業共同體。" lead="會員資格、類型、費用、權利義務與入會程序均應依正式章程及最新會務決議；網站不把會員制度包裝成商業服務方案。"/><section className="g4-section"><div className="g4-wrap"><div className="g4-grid">{[["個人會員","年滿二十五歲、贊同宗旨，依章程完成申請與審查。","入會費 NT$2,000｜常年會費 NT$2,000"],["榮譽會員","由學會依章程與正式程序邀請之公私立機構、團體或個人。","資格與權利義務以章程為準"],["贊助會員","贊同宗旨且對學會無償捐贈之個人、機構或團體。","資格與權利義務以章程為準"]].map((x,i)=><div className="g4-card" key={x[0]}><span>0{i+1}</span><h3>{x[0]}</h3><p>{x[1]}</p><p style={{color:'#835c2e'}}>{x[2]}</p></div>)}</div><div className="g4-kicker" style={{marginTop:56}}>APPLICATION FLOW</div><h2 className="g4-title">入會流程</h2><div className="g4-steps">{[["01","提出申請與必要資料"],["02","依章程與會務程序進行資格審查"],["03","完成應繳費用與必要程序"],["04","會員資格成立後進入正式會務與活動通知"]].map(([n,t])=><div className="g4-step" key={n}><b>{n}</b><p>{t}</p></div>)}</div><div className="g4-note"><h3>會員權利與義務</h3><p>會員之參與、表決、遵守章程、繳納會費及其他權利義務，均依章程及正式會務決議辦理。</p><div className="g4-actions"><a className="primary" href={LINE_URL} target="_blank" rel="noreferrer">入會與會務聯絡 ↗</a></div></div></div></section></div>}

function Events(){return <div className="g4-page"><PageHead eyebrow="ASSEMBLY & PROGRAMS" title="活動不是一次性聚會，而是學會制度運作與知識累積的正式紀錄。" lead="會員大會、理監事會、專題講座與正式交流，在資料可核對後逐年建立活動檔案；沒有正式資料的活動不預先虛構。"/><section className="g4-section"><div className="g4-wrap"><div className="g4-event"><time>2026.08.16</time><div><h3>第一屆第二次會員大會暨第三次理監事聯席會議</h3><p>本次活動結合理監事聯席會議、會員交流、專題講座與餐敘；專題講座主題為「從企業策略到永續治理」。</p></div><span className="g4-badge">FORMAL ASSEMBLY</span></div><div className="g4-note"><h3>活動檔案原則</h3><p>每一場正式活動頁至少保留日期、活動名稱、性質、正式議程、核定照片與可公開內容；不以未核對的成果數字、合作名單或媒體曝光作為裝飾。</p></div></div></section></div>}

function Knowledge(){return <div className="g4-page"><PageHead eyebrow="KNOWLEDGE & RESEARCH" title="讓活動與交流逐步沉澱為可以再次學習、引用與傳承的治理知識。" lead="本頁作為學會知識資產入口，整理企業策略、公司治理、法遵、風險控管與永續相關之可公開研究、講座與治理內容。"/><section className="g4-section"><div className="g4-wrap"><div className="g4-grid">{[["01","研究與治理議題","逐步整理企業策略、公司治理、法遵、風險控管與永續相關研究議題。"],["02","專題講座","在講座正式完成且資料可公開後，建立主題、講者、摘要與活動紀錄。"],["03","知識資產","把會員交流、正式會議與研究討論中可公開的內容，逐步整理為索引與長期資料。"]].map(x=><div className="g4-card" key={x[0]}><span>{x[0]}</span><h3>{x[1]}</h3><p>{x[2]}</p></div>)}</div><div className="g4-note"><h3>知識紀錄原則</h3><p>研究、出版與可公開文件均以已完成且可核對之資料為準，不以虛構篇數、合作機構、研究計畫或影響力數字替代實際成果。</p></div></div></section></div>}

function Charter(){return <div className="g4-page"><PageHead eyebrow="CHARTER & NOTICE" title="章程不是網站附件，而是學會權力來源、會員權利義務與制度運作的正式依據。" lead="本頁呈現可由現有會務資料確認的章程摘要與法定立案資訊；章程全文、公告與會務文件僅收錄已正式核定且可公開之版本。"/><section className="g4-section"><div className="g4-wrap"><div className="g4-list">{charterHighlights.map(([a,b])=><div className="g4-row" key={a}><b>{a}</b><div>{b}</div></div>)}</div><div className="g4-note"><h3>組織法定立案</h3><p>內政部 114/8 台內團字第 1140030747 號。</p></div><div className="g4-note"><h3>公告與正式文件</h3><p>此區只收錄已正式核定、可公開之章程、公告、會員大會或理監事會相關文件；網站說明文字不取代正式法定文件。</p></div></div></section></div>}

function Privacy(){return <div className="g4-page"><PageHead eyebrow="PRIVACY" title="隱私與資料使用" lead="學會網站以會務、會員聯絡與活動資訊為主要用途；網站僅就實際啟用之資料收集與第三方服務說明其使用範圍。"/><section className="g4-section"><div className="g4-wrap"><div className="g4-list"><div className="g4-row"><b>會員資料</b><div>僅在辦理入會、會務聯絡、活動通知與依法所需範圍內使用；正式資料項目與保存期限應依實際制度揭露。</div></div><div className="g4-row"><b>外部連結</b><div>LINE、STT Governance 或其他第三方網站之資料處理由其各自服務條款與隱私規則管理。</div></div><div className="g4-row"><b>資料使用原則</b><div>Cookie、流量分析、表單、第三方嵌入與主機紀錄等項目，僅在實際啟用時依其實際用途與服務條件揭露。</div></div></div></div></section></div>}
