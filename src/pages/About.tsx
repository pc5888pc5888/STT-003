...
function ContactModal({ onClose }: { onClose: () => void }) {
  ...
  const inputClass = "w-full bg-zinc-900 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-zinc-500 focus:outline-none focus:border-[#e6c84c]/60 transition-all";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)" }}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
          <div>
            <p className="text-[#e6c84c] font-mono tracking-widest uppercase font-bold" style={{ fontSize: "9px" }}>CONTACT STT — INITIATE COREGULATION</p>
            <h3 className="text-white font-serif font-bold mt-0.5" style={{ fontSize: "18px" }}>聯絡智庫</h3>
          </div>
          <button type="button" onClick={onClose} className="text-zinc-500 hover:text-white transition-colors cursor-pointer bg-transparent border-0 outline-none p-1">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="px-6 py-5 space-y-4">
          {status === "success" ? (
            <div className="text-center py-10">
              ...
              <p className="text-zinc-400 mt-1" style={{ fontSize: "13px" }}>莊鈞翔博士團隊將盡快與您聯繫。</p>
              <button type="button" onClick={onClose} className="mt-6 px-6 py-2 border border-[#e6c84c]/40 text-[#e6c84c] rounded-lg hover:bg-[#e6c84c]/10 transition-all cursor-pointer bg-transparent outline-none" style={{ fontSize: "13px" }}>關閉</button>
            </div>
          ) : (
            <>
              ...
              <div>
                ...
                <textarea className={inputClass} style={{ fontSize: "13px", resize: "none" }} name="message" value={form.message} onChange={handleChange} rows={4} placeholder="請簡述您的需求或問題..." />
              </div>
              {status === "error" && (
                <p className="text-red-400 text-center" style={{ fontSize: "12px" }}>送出失敗，請稍後再試或直接寄信至 pc5888@gmail.com</p>
              )}
              <button type="button" onClick={handleSubmit} disabled={status === "sending"}
                ...
                style={{ fontSize: "14px" }}>
                {status === "sending" ? (<><Loader2 className="w-4 h-4 animate-spin" />送出中...</>) : (<><Send className="w-4 h-4" />送出申請</>)}
              </button>
            </>
          )}
        </div>
                  ...
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <div className="border-l-2 border-[#e6c84c]/75 bg-zinc-950/45 flex-shrink-0" style={{ padding: 'clamp(8px, 1vh, 14px)', marginBottom: 'clamp(6px, 1.2vh, 14px)' }}>
                        <p className="font-sans text-[#EBE7DF]/90 tracking-wider italic" style={{ fontSize: 'clamp(10px, 0.95vw, 14px)', lineHeight: '1.7' }}>我們絕不推銷任何廉價的建議，我們僅為真正珍視基業與秩序的領袖，建立無懈可擊的法商防火牆。</p>
                      </div>
                      ...
                      <p className="text-[#e6c84c] font-serif italic tracking-wide flex-shrink-0 mt-auto" style={{ fontSize: 'clamp(10px, 0.9vw, 13px)' }}>為你的內心，打造一座不可侵犯的至聖所<span className="border-b border-[#e6c84c]/50 pb-0.5 ml-1">--- 摘自《Internal Compliance》莊鈞翔博士著</span></p>
                    </div>
                  ...
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <div className="grid grid-cols-2 gap-2 flex-1 overflow-hidden">
                        ...
                        <Card label="MEDIA COLUMNIST" title="M傳媒 法遵智庫專欄主筆">
                          <p className="text-zinc-300 font-light leading-relaxed mt-1">定期發表法遵、治理、家族接班與企業策略的深度分析。</p>
                        </Card>
                        <Card label="ACADEMIC APPOINTMENT" title="逢甲大學商學院兼任助理教授">
                          <p className="text-zinc-300 font-light leading-relaxed mt-1">開設企業治理與法遵課程，結合 AI 治理與 ESG 實務。</p>
                        </Card>
                      </div>
                    </div>
                ...
                {currentSlide === 2 && (
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <p className="text-stone-300 tracking-wide flex-shrink-0" style={{ fontSize: 'clamp(10px, 0.85vw, 13px)', marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>學術嚴謹性與產業實踐並重</p>
                      <div className="grid grid-cols-2 gap-2 flex-shrink-0" style={{ marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>
                        <Card label="ACADEMIC FOUNDATION" title="學歷背景">
                          <div className="mt-1.5 space-y-2 text-zinc-300">
                            <div><span className="font-semibold text-white">▸ 管理學博士 Ph.D.</span><p className="text-zinc-400 pl-2 mt-0.5">逢甲大學管理學院 (2024)</p></div>
                            <div><span className="font-semibold text-white">▸ 嶺東科技大學 EMBA</span></div>
                          </div>
                        </Card>
                        <Card label="EMPIRICAL RIGOR" title="代表著作">
                          <div className="mt-2 space-y-1.5 text-zinc-300">
                            <p className="text-white font-semibold">▸《內在法遵 Internal Compliance》</p>
                            <p className="text-white font-semibold">▸《2025 永續家族治理實務實錄》</p>
                            <p className="text-white font-semibold">▸《臺灣企業接班人的佈局規劃與傳承家族價值》</p>
                          </div>
                        </Card>
                      </div>
                      <div className="border-l-2 border-[#e6c84c]/75 bg-zinc-950/45 flex-shrink-0" style={{ padding: 'clamp(8px, 1vh, 14px)', fontSize: 'clamp(10px, 0.9vw, 13px)' }}>
                        <p className="text-[#dbd7cf] font-light italic leading-relaxed">學術是我看透制度本質的透鏡，實務是我驗證理論的試煉場。-- CHUANG CHUN HSIANG Ph.D.</p>
                      </div>
                    </div>
                ...
                {currentSlide === 3 && (
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <p className="text-stone-300 tracking-wide flex-shrink-0" style={{ fontSize: 'clamp(10px, 0.85vw, 13px)', marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>跨國機構認可的專業資格</p>
                      <div className="space-y-2 flex-shrink-0" style={{ fontSize: 'clamp(10px, 0.9vw, 13px)', marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>
                        <p className="text-[#dbd7cf] leading-relaxed">▸ <strong className="text-white">Anthropic 與 Google 核發 AI 專業認證</strong>：涵蓋 Claude 101、Amazon Bedrock、Gemini AI 等認證。</p>
                        <p className="text-[#dbd7cf] leading-relaxed">▸ <strong className="text-white">TÜV 萊茵集團永續發展認證</strong>：ESG 永續評估師、碳足跡查驗師等資格。</p>
                      </div>
