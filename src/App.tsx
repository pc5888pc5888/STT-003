...
function PublicShell({ children, chatOpen, onChatToggle }: ShellProps) {
  ...
  const primaryNavigation = useMemo(() => [
    ...
    { label: "人文地景產採訪", path: "/humanistic-20q" },
    { label: "關於 STT", path: "/stt" },
    { label: "預約與合作", path: "/cooperation" },
  ], []);
    ...
    <div className="min-h-screen" style={{ background: "#fbfaf7", color: "var(--stt-ink)" }}>
      <header className="sticky top-0 z-[70] border-b bg-white/95 backdrop-blur-xl" style={{ borderColor: "var(--stt-line)", minHeight: "var(--stt-header-height)" }}>
        <div className="mx-auto flex h-[76px] max-w-[1320px] items-center px-5 lg:px-8">
          <button type="button" onClick={() => go("/")} className="stt-official-brand mr-auto min-w-0 flex-1 cursor-pointer border-0 bg-transparent p-0 text-left" aria-label="STT Governance 首頁">
            <img src={STT_OFFICIAL_LOGO_SRC} alt="STT Governance Strategic Think Tank Governance｜策略智庫｜治理出版｜法遵精神" className="stt-official-logo block h-auto w-full object-contain object-left" />
          </button>
          <nav className="hidden h-full items-center 2xl:flex" aria-label="Primary">
            {primaryNavigation.map((item) => {
              ...
              return <button key={item.path} type="button" onClick={() => go(item.path)} className="relative h-full cursor-pointer border-0 bg-transparent px-3 text-[11px] tracking-[0.04em]" aria-current={active ? "page" : undefined} style={{ color: active ? "var(--stt-gold-deep)" : "var(--stt-ink-soft)" }}>{item.label}{active && <span className="absolute bottom-0 left-3 right-3 h-px" style={{ background: "var(--stt-gold)" }} />}</button>;
            })}
          </nav>
          <div className="ml-4 hidden items-center gap-3 xl:flex">
            <button type="button" onClick={() => go("/cooperation")} className="inline-flex cursor-pointer items-center gap-2 border bg-transparent px-4 py-2.5 text-xs font-semibold" style={{ borderColor: "var(--stt-gold-line)", color: "var(--stt-gold-deep)" }}>預約與合作<ArrowRight className="h-3.5 w-3.5" strokeWidth={1.3} /></button>
...
function AppRoutes() {
  return (
    <Routes>
      ...
      <Route path="/contact.html" element={<Navigate to="/cooperation" replace />} />
