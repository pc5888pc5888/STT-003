...
const pages: Record<PageKind, PageConfig> = {
  ...
  projects: {
    eyebrow: "HUMANISTIC & GOVERNANCE MEMORY", title: "有些治理資產，只能在人還能說、還能修正時留下。",
    ...
    finalAction: "說明專案需求", finalPath: "/start?route=founder-legacy",
  },
  stt: {
...
function CanonicalPage({ kind }: { kind: PageKind }) {
  ...
  return <div className="stt-canon">
    ...
    <section className="stt-canon-body"><div className="stt-canon-wrap">
      ...
      <div className="stt-canon-grid">{page.sections.map((section) => <article className="stt-canon-card" id={kind === "method" ? `judgment-step-${Number(section.number)}` : undefined} key={section.number}>
    ...
    {kind === "stt" && <STTRoles />}
    <section className="stt-canon-final"><div className="stt-canon-wrap"><p className="stt-master-kicker">FINAL JUDGMENT</p><h2>{page.finalTitle}</h2><p>{page.finalBody}</p>{page.finalAction && page.finalPath && <button type="button" onClick={() => navigate(page.finalPath!)}>{page.finalAction} →</button>}</div></section>
  </div>;
}
