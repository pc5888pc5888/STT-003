...
const HUMANISTIC_INTERVIEW_URL = "/humanistic-interview/index.html";
...
function PublicShell({ children, chatOpen, onChatToggle }: ShellProps) {
  ...
  const primaryNavigation = useMemo(() => [
    ...
    { label: "人文地景產採訪", path: "/humanistic-interview" },
        ...
        <div className="mx-auto flex h-[76px] max-w-[1320px] items-center px-5 lg:px-8">
          ...
          <nav className="hidden h-full items-center 2xl:flex" aria-label="Primary">
            {primaryNavigation.map((item) => {
              ...
              return <button key={item.path} type="button" onClick={() => go(item.path)} className="relative h-full cursor-pointer border-0 bg-transparent px-2.5 text-[10.5px] tracking-[0.035em]" aria-current={active ? "page" : undefined} style={{ color: active ? "var(--stt-gold-deep)" : "var(--stt-ink-soft)" }}>{item.label}{active && <span className="absolute bottom-0 left-2.5 right-2.5 h-px" style={{ background: "var(--stt-gold)" }} />}</button>;
            })}
          </nav>
...
function AppRoutes() {
  return (
    <Routes>
      ...
      <Route path="/humanistic-interview" element={<ExternalRedirect url={HUMANISTIC_INTERVIEW_URL} />} />
