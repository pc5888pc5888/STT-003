...
function PublicShell({ children, chatOpen, onChatToggle }: ShellProps) {
  ...
  const primaryNavigation = useMemo(() => [
    ...
    { label: "洞察與專欄", path: "/insights" },
    { label: "出版與研究", path: "/publications" },
      ...
      <header className="sticky top-0 z-[70] border-b bg-white/95 backdrop-blur-xl" style={{ borderColor: "var(--stt-line)", minHeight: "var(--stt-header-height)" }}>
        <div className="mx-auto flex h-[76px] max-w-[1320px] items-center px-5 lg:px-8">
          <button type="button" onClick={() => go("/")} className="mr-auto cursor-pointer border-0 bg-transparent p-0 text-left">
            ...
            <span className="mt-1 block text-[8px] uppercase tracking-[0.28em]" style={{ color: "var(--stt-gold-deep)" }}>Strategy · Governance · Judgment</span>
          </button>
          ...
          <div className="ml-4 hidden items-center gap-3 xl:flex">
            <button type="button" onClick={() => go("/start")} className="inline-flex cursor-pointer items-center gap-2 border bg-transparent px-4 py-2.5 text-xs font-semibold" style={{ borderColor: "var(--stt-gold-line)", color: "var(--stt-gold-deep)" }}>開始治理判讀<ArrowRight className="h-3.5 w-3.5" strokeWidth={1.3} /></button>
