...
export default function GovernanceResponsibility(){
  ...
  return <div className="gr-root">
    ...
    <section className="gr-section"><div className="gr-wrap"><p className="stt-master-kicker">CURRENT POSITIONS</p><h2>目前公開職務</h2><div className="gr-list">{positions.map((p,i)=><div className="gr-row" key={p}><span>{String(i+1).padStart(2,'0')}</span><p>{p}</p></div>)}</div><p className="gr-desc">網站正式上線前，公開職務與外部機構名稱仍應逐項對照最新任職資料；若職務變動，網站同步更新，不以舊版簡歷永久固定。</p></div></section>
