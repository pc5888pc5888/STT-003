      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
          <div>
            ...
            <h3 className="text-white font-serif font-bold mt-0.5" style={{ fontSize: "18px" }}>?舐窗?箏澈</h3>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          {status === "success" ? (
            <div className="text-center py-10">
              ...
              <p className="text-zinc-400 mt-1" style={{ fontSize: "13px" }}>??蝧?憯怠????∪翰??舐鼠??/p>
              <button type="button" onClick={onClose} className="mt-6 px-6 py-2 border border-[#e6c84c]/40 text-[#e6c84c] rounded-lg hover:bg-[#e6c84c]/10 transition-all cursor-pointer bg-transparent outline-none" style={{ fontSize: "13px" }}>??</button>
            </div>
          ) : (
            <>
              ...
              <div>
                ...
                <textarea className={inputClass} style={{ fontSize: "13px", resize: "none" }} name="message" value={form.message} onChange={handleChange} rows={4} placeholder="隢陛餈唳??瘙???..." />
              </div>
              {status === "error" && (
                <p className="text-red-400 text-center" style={{ fontSize: "12px" }}>?憭望?嚗?蝔??岫??亙?靽∟ pc5888@gmail.com</p>
              )}
              <button type="button" onClick={handleSubmit} disabled={status === "sending"}
                ...
                style={{ fontSize: "14px" }}>
                {status === "sending" ? (<><Loader2 className="w-4 h-4 animate-spin" />?銝?..</>) : (<><Send className="w-4 h-4" />??唾?</>)}
              </button>
            </>
          )}
        </div>
                  ...
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <div className="border-l-2 border-[#e6c84c]/75 bg-zinc-950/45 flex-shrink-0" style={{ padding: 'clamp(8px, 1vh, 14px)', marginBottom: 'clamp(6px, 1.2vh, 14px)' }}>
                        <p className="font-sans text-[#EBE7DF]/90 tracking-wider italic" style={{ fontSize: 'clamp(10px, 0.95vw, 14px)', lineHeight: '1.7' }}>??銝?瑚遙雿??寧?撱箄降嚗????箇?甇??閬璆剛?蝘拙???鋡?撱箇??⊥??舀?????怎???/p>
                      </div>
                      ...
                      <p className="text-[#e6c84c] font-serif italic tracking-wide flex-shrink-0 mt-auto" style={{ fontSize: 'clamp(10px, 0.9vw, 13px)' }}>?箔??敹???摨找??臭噩?舐??唾??<span className="border-b border-[#e6c84c]/50 pb-0.5 ml-1">--- ??nternal Compliance?????ㄚ??/span></p>
                    </div>
                  ...
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <div className="grid grid-cols-2 gap-2 flex-1 overflow-hidden">
                        ...
                        <Card label="MEDIA COLUMNIST" title="M?喳? 瘜?箏澈撠?銝餌?">
                          <p className="text-zinc-300 font-light leading-relaxed mt-1">摰??潸”瘜?祥?振??剛?隡平蝑?楛摨血???/p>
                        </Card>
                        <Card label="ACADEMIC APPOINTMENT" title="?Ｙ憭批飛?飛?Ｗ隞餃????>
                          <p className="text-zinc-300 font-light leading-relaxed mt-1">?身隡平瘝餌????菔玨蝔?蝯? AI 瘝餌???ESG 撖血???/p>
                        </Card>
                      </div>
                    </div>
                ...
                {currentSlide === 2 && (
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <p className="text-stone-300 tracking-wide flex-shrink-0" style={{ fontSize: 'clamp(10px, 0.85vw, 13px)', marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>摮貉??渲牲?扯??Ｘ平撖西?銝阡?</p>
                      <div className="grid grid-cols-2 gap-2 flex-shrink-0" style={{ marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>
                        <Card label="ACADEMIC FOUNDATION" title="摮豢風?">
                          <div className="mt-1.5 space-y-2 text-zinc-300">
                            <div><span className="font-semibold text-white">??蝞∠?摮詨?憯?Ph.D.</span><p className="text-zinc-400 pl-2 mt-0.5">?Ｙ憭批飛蝞∠?摮賊 (2024)</p></div>
                            <div><span className="font-semibold text-white">??撊箸蝘?憭批飛 EMBA</span></div>
                          </div>
                        </Card>
                        <Card label="EMPIRICAL RIGOR" title="隞?”??">
                          <div className="mt-2 space-y-1.5 text-zinc-300">
                            <p className="text-white font-semibold">?詻?冽???Internal Compliance??/p>
                            <p className="text-white font-semibold">?詻?025 瘞貊?摰嗆?瘝餌?撖血?撖阡???/p>
                      ...
                      <div className="border-l-2 border-[#e6c84c]/75 bg-zinc-950/45 flex-shrink-0" style={{ padding: 'clamp(8px, 1vh, 14px)', fontSize: 'clamp(10px, 0.9vw, 13px)' }}>
                        <p className="text-[#dbd7cf] font-light italic leading-relaxed">摮貉??舀??摨行鞈芰??嚗祕???霅?隢?閰衣??氬?- CHUANG CHUN HSIANG Ph.D.</p>
                      </div>
                    </div>
                ...
                {currentSlide === 3 && (
                  <div className="flex flex-row" style={{ height: '100%', overflow: 'hidden' }}>
                    <div className="w-full lg:w-[58%] flex flex-col overflow-hidden flex-shrink-0" style={{ padding: 'clamp(8px, 1.5vh, 24px) 0', paddingRight: 'clamp(16px, 2vw, 40px)' }}>
                      ...
                      <p className="text-stone-300 tracking-wide flex-shrink-0" style={{ fontSize: 'clamp(10px, 0.85vw, 13px)', marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>頝典?璈?隤??璆剛???/p>
                      <div className="space-y-2 flex-shrink-0" style={{ fontSize: 'clamp(10px, 0.9vw, 13px)', marginBottom: 'clamp(8px, 1.2vh, 16px)' }}>
                        <p className="text-[#dbd7cf] leading-relaxed">??<strong className="text-white">Anthropic ??Google ?貊 AI 撠平隤?</strong>嚗項??Claude 101?mazon Bedrock?emini AI 蝑?霅?/p>
                        <p className="text-[#dbd7cf] leading-relaxed">??<strong className="text-white">T?V ???瘞貊??澆?隤?</strong>嚗SG 瘞貊?閰摯撣怒４頞唾楚?仿?撣怎?鞈??/p>
                      </div>
